...
prone to landslides amid natural changes to that of the geology, tropical factors, environmental factors and many more. In the process of mapping these regions and classifying them into different categories of threat level, it helps in early detection and avoidance. By using EOBrowser [3] to quickly view and analyze different algorithms, this study demonstrates the potential for more efficient and accurate landslide mapping. After mapping existing landslide cases, those cases were incorporated during the data collection process and further fed into a
Conditional Adversarial Network for predictive landslide susceptibility mapping. During data collection process various factors were
cross referenced and: NDVI, dNDVI, slope, elevation, NDWI, and the bare soil index were ranked as the most important contributing factors.
...
\begin{figure}[htbp]
    ...
    \caption{Comparison between the proposed algorithm and the Landslide Atlas of India [1]}
...
\section{Dataset Features and Factors}
...
Through experimental evaluation, it was observed that NDVI, dNDVI, slope, elevation, NDWI, and bare soil index were sufficient to achieve high accuracy. NDVI, dNDVI, NDWI, Barren soil index are retrieved from Sentinel satellite via API and Slope and elevation maps are retrieved from Copernicus DEM (Digital Elevation Model). Scripts written in EO Browser provide an instant preview of the LSM but the final rendering of LSM happens on codebase where other factors such as dNDVI (which is difference Normalized Vegetation Index) that cannot be calculated as dNDVI requires multi-temporal (multi-date) analysis, while EO Browser primarily processes data for single timestamps at a time are applied.\par
Furthermore, the Conditional Adversarial Network was developed by inheriting the basic architecture of “Image-to-Image Translation with Conditional Adversarial Networks” paper, This paper was designed to accept on image as guidance, Adapting the model to the proposed requirements necessitated modification of the input mechanism that allowed multiple images to  be accepted as inputs, that leads to the generating of a mask, better understanding of images and faster convergence since the original models were found.
...
This is crucial in landslide or forest fire detection task as it is a vital point from analysis point of view. By finding dNDVI a mask can be generated as mentioned before.
...
By subtracting and retaining only those pixels whose NDVI (after landslide) is lower than that of NDVI (before landslide), to make sure the mask obtained is of vegetation decreasing and not vegetation increasing [10].\par
...
In this implementation, n = 5, i.e the number of input to the cGAN is 5, Additional input channels can be incorporated; however, excessive conditioning may lead to diminishing returns as there will be a saturation point that a certain number of input the Conditions can become counterproductive.\par
L1 loss is employed, consistent with prior work, that is Lasso (Least Absolute Shrinkage and Selection Operator), this encourages sparsity, meaning it tends to drive some coefficients exactly to zero.
...
A PatchGAN discriminator is employed – that only penalizes structure at the scale of patches. This discriminator tries to classify if each N×N patch in an image is real or fake. The discriminator is applied convolutionally across the image, averaging all responses to provide the ultimate output of D.
...
the output. Considering these factors, the generator architecture is designed accordingly. \par
...
Dataset creation was a computationally intensive component of the workflow and existing datasets were not suitable for the objectives of this study. To
accomplish this Landslide data were obtained from NASA’s dataset [14], out of which the relevant columns selected were: event-date (The date on which the landslide occurred), event-title ( brief title describing the event), latitude, longitude.
...
the analysis is based on recent events and The selection of 2018 as the starting year is due to the operational consistency of Sentinel satellites is that sentinel satellites
became operational after 2017, and consistent somewhere around late 2017.


\subsubsection{Automating Grid Scan Algorithm}
The approach used to retrieve large geographic regions for scanning when required and scaling is back down for various
requirements while minimizing data loss A grid-based approach is used, with each cell accessed sequentially. Future enhancements may include parallelization of this approach.
...
\begin{figure}[htbp]
    ...
    \caption{An example of how the grid scan algorithm segments a piece of land (southern India for reference).}
    \label{fig:12}
\end{figure}
...
The reason for the first row of the grid to be above the user defined longitude and longitude is that during the analysis,
constantly using grid of 1x1 for scanning smaller pieces of land, which was justified when longitude and latitude were bottom
left, this approach can be altered but this approach was adopted.\par
...
A challenge identified during dataset creation was cloud contamination, Clouds pose a threat to
...
flies over the desired region, being every 5-6 days, using Copernicus’s Catalog API [15] it made it possible to fetch 5-6
...
User has the option of whether to apply these steps for each grid separately or altogether, for this paper, these steps were applied independently to each grid.\par
...
From these projects (approximately 900), the dataset was constructed [Fig 13]\par
...
Model training closely follows the Pix2Pix framework but few more metrics have been incorporated into it such as
...
The trained model achieved an accuracy of 0.899.
